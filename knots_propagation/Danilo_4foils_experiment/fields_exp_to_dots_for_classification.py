...
if __name__ == "__main__":
	...
	flower = '2221'
	folder_path = 'C:/Users/Cmex-/Box/Flowers Exp/T_30/'  # Path to the folder containing .mat files with experimental data
	folder_save = 'all_flowers'
	if not os.path.exists(f'./{folder_save}'):
		os.makedirs(f'./{folder_save}')
	# 3.1623e-04 is the beam size in the experiment
	# window_size = 6.4 * 1e-3 * 6e-3 / np.sqrt(2) / 3.1623e-4
	window_size = 6.4 * 1e-3 * np.sqrt(270/1.5)
	window_size = 4.6 * 1e-3 * np.sqrt(270/1.5)

	print(f'experimental window_size: {window_size}')


	# crop_size = 300  # Size of the cropped region (centered on the center of mass), in pixels
	rescale_size = 185  # Resolution to rescale the cropped region (square size)
	saving_size = (32, 32)  # Final resolution for extracted singularities (dots) for saving
	res_z = 64  # Number of propagation steps along the z-axis for 3D field expansion
	saving_knot = [32, 32, res_z]
	crop_3d = 100  # Size of the cropped 3D volume (in x and y dimensions)
	xy_lim_2D_origin = np.array((-30.0e-3, 30.0e-3)) # Physical bounds of the 2D field in meters (x and y axes)
	knot_length = 212.58897655870774 / 2 * 1.4  # Length of the knot used during beam expansion
	
	needed_size = (xy_lim_2D_origin[1] - xy_lim_2D_origin[0]) * 185 / 300
	# Process the folder to get the list of tensors
	processed_fields = process_folder(folder_path, window_size, needed_size, rescale_size, one=plot)

	# Beam and wave parameters
	lmbda = 532e-9  # Wavelength of the light source, in meters
	width0 = 6.0e-3 / np.sqrt(2)  # Initial beam width at the source, in meters
	res_xy_2D_origin = 300  # Resolution for the 2D field during initial processing
	k0 = 2 * np.pi / lmbda  # Wave number (related to wavelength)
	
	# Pixel scaling and beam expansion parameters
	pxl_scale = (xy_lim_2D_origin[1] - xy_lim_2D_origin[0]) / (res_xy_2D_origin - 1)
	# Physical size of one pixel in the 2D grid, in meters
