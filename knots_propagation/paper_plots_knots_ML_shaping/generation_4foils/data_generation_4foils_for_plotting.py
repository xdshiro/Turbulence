...
for Rytov in Rytovs:
    ...
    for foil in foils:
        ...
        for indx in trange(SAMPLES, desc="Progress"):
            ...
            plot_field_both(field_z_crop, extend=extend_crop)
            if save:
                np.save(f'foil4_field_XY_{Rytov}_{foil}.npy', field_z_crop)
            if spectrum_save:
                if centering:
                ...
                ###########
                if save:
                    np.save(f'foil4_spectr_XY__{Rytov}_{foil}.npy', spectrum)
            ...
            if save:
                np.save(f'foil4_dots_{Rytov}_{foil}.npy', dots_cut)
