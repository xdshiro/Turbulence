...
def save_intensity_gif(intensity_data, output_filename='intensity_movie.gif'):
    """
    Saves a 3D intensity array as a GIF using only PIL.

    Parameters:
    - intensity_data (numpy.ndarray): A 3D array of shape (num_frames, height, width) containing the intensity data.
    ...
    if len(intensity_data.shape) != 3:
        raise ValueError("Intensity data must be a 3D array with shape (num_frames, height, width).")

    # Normalize intensity data to 0-255 range for GIF
    min_val = np.min(intensity_data)
    max_val = np.max(intensity_data)
    intensity_normalized = 255 * (intensity_data - min_val) / (max_val - min_val)
    intensity_normalized = intensity_normalized.astype(np.uint8)

    images = [Image.fromarray(frame) for frame in intensity_normalized]

    # Save the frames as a GIF
    images[0].save(output_filename, save_all=True, append_images=images[1:], loop=0, duration=500)
...
def run_simulation(L_prop, width0, xy_lim_2D, res_xy_2D, Rytov, l0, L0, screens_nums, knot_length):
    ...
    # SR, currents_SR = SR_gauss_fourier(mesh_2D, L_prop, beam_par, psh_par, epochs=3000, screens_num=screens_nums, max_cut=False, pad_factor=4)
    # SR_reversed = SR_reversed_gauss_fourier(mesh_2D, L_prop, beam_par, psh_par, epochs=100, screens_num=screens_nums, max_cut=False, pad_factor=4)
    # SR_reversed = SR_center_gauss_fourier(mesh_2D, L_prop, beam_par, psh_par, epochs=100, screens_num=screens_nums, max_cut=False, pad_factor=4)

    scin, currents = scintillation(mesh_2D, L_prop, beam_par, psh_par, epochs=100, z=-L_prop * 0, screens_num=screens_nums, seed=None)
    # plot_field_both(currents[0][100:200, 100:200])
    # plot_field_both(currents[1][100:200, 100:200])
    save_intensity_gif(np.array(currents)[:, 100:200, 100:200],
                       output_filename=f'intensity_movie_diverge_{Rytov}.gif')
    # print(currents)# scin_middle = scin[res_xy_2D // 2, res_xy_2D // 2]
