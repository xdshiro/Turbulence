...
def field_propagator(field, k0, dz, dx, dy):
	...
	field_prop = np.fft.ifft2(np.fft.fftshift(field_ft_prop))
	
	# Return the propagated field
	return field_prop


width0 = 5e-3 / np.sqrt(2)  # Beam width (related to the beam waist or size of the beam)
...
z_lim_3D = (0.0001, 90)  # Z boundaries (in arbitrary units, typically represents propagation distance)
# z_lim_3D = (0.0001, 100)
...
res_z_3D = 120  # Number of points along the Z-axis
...
for i, z in enumerate(z_3D):
	...
	field_xz[:, i] = field_current[:, res_y_3D // 2]
