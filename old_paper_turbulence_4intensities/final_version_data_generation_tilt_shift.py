...
save_danilo = True
save_folder = './Danilo_tilt/'
angle_tilt = 2.5e-5
angle_tilts = [1e-6, 5e-6, 1e-5, 2.5e-5, 5e-5]
for angle_tilt in angle_tilts:
    save_name = f'tilt_angle_{angle_tilt}'
    ...
    for Rytov in Rytovs:
        ...
        folder = f'tilt_shift_{Rytov}_optimized_test'
        ...
        for knot in knots:
            ...
            for indx in trange(SAMPLES, desc="Progress"):
                ...
                ###########################################################
                ###########################################################
                # tilt_angle = 0  # Tilt angle in degrees
                X_tilt = mesh_2D_original[0]
                dx = x_2D_origin[1] - x_2D_origin[0]
                Lx = x_2D_origin[-1] - x_2D_origin[0]
                # print(Lx)
                # k_x_tilt = 2 * np.pi * np.sin(np.radians(tilt_angle)) / Lx
                k_x_tilt = angle_tilt * k0
                phase_gradient_tilt = np.exp(1j * k_x_tilt * X_tilt)
                field_center = field_center * phase_gradient_tilt
                plot_field_both(field_center, extend=extend)
                ...
                if plot_3d:
                    ...
                    pl.plotDots(dots_cut, dots_bound, color='black', show=False, size=10, save=save_path+'.html')
                print(dots_cut - np.array([crop_3d, crop_3d, res_z + 1]) // 2)
                data_to_save = {'field_center': field_center,
                                'dots': dots_cut - np.array([crop_3d, crop_3d, res_z + 1]) // 2}
                savemat(save_path+"_2.mat", data_to_save)
