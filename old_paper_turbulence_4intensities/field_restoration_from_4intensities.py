...
for file_name in os.listdir(input_folder):
	if file_name.endswith('.mat'):
		...
		print(file_name)
		U_f_resized = process_knot_fields(file_path, plot=True, reso=reso)
		continue
		if plotted < plot:
			plot_field_both(U_f_resized)
			plotted += 1
		...
		# Save dots_cut
		output_file_path = os.path.join(output_folder, f'dots_{file_name}')
